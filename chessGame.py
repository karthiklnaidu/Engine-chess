...
class GamePlay:
    ...
    def anyPieceAtMiddle(self, attacker_square, target_square):
            ...
            while (current_file != target_file or current_rank != target_rank):
                ...
                if square in board.board and board.board[square]['piece'] is not None:
                    return True
    ...
    def is_able_to_obstruct(self, player_color, king_position, attacker_position):
        target = board.board[king_position]
        attacker = board.board[attacker_position]
    ...
    def pawn_can_attack(self, pawn_position, target_square):
        ...
        if board[pawn_position]['piece-color'] == 'white':
        ...
        if abs(file_difference) == 1 and rank_difference == direction:
            target_square_piece = board[target_square]['piece']
            # Check if there is a piece of the opponent's color on the target square
            if target_square_piece is not None and board[target_square]['piece-color'] != board[pawn_position]['piece-color']:
                return True
        return False
    ...
    def make_castle(self, king_position, rook_position):
        def make_move(king_to, rook_to):
            board[king_position]['piece'] = None
            board[rook_position]['piece'] = None
            board[king_position]['piece-color'] = None
            board[rook_position]['piece-color'] = None
            board[king_to]['piece'] = 'king'
            board[rook_to]['piece'] = 'rook'
    ...
    def pawn_promotion(self, promoting_square):
        ...
        board.board[promoting_square]['piece'] = select_piece_to_promote
    ...
    def move_piece(self, from_square = None, to_square = None, special_move = None):
        ...
        if self.is_valid_move(from_square, to_square):
            piece_to_move = board[from_square]['piece']
            self.make_move(piece_to_move, board[from_square], board[to_square])

            king_position = getattr(self, f'{self.current_player}_king')
            if(self.is_check(self.current_player, king_position.position)):
                self.make_move(piece_to_move, board[to_square], board[from_square])
    ...
    def is_valid_move(self, from_square, to_square):
        if board[from_square]['piece'] == 'pawn':
            for pawn in getattr(self, f'{self.current_player}_pawn'):
                if pawn.position == from_square:
                    return pawn.move(self.current_player, game, board[from_square], board[to_square])
        elif board[from_square]['piece'] == 'knight':
            for knight in getattr(self, f'{self.current_player}_knight'):
                if knight.position == from_square:
                    return knight.move(self.current_player, game, board[from_square], board[to_square])
        elif board[from_square]['piece'] == 'bishop':
            for bishop in getattr(self, f'{self.current_player}_bishop'):
                if bishop.position == from_square:
                    return bishop.move(self.current_player, game, board[from_square], board[to_square])
        elif board[from_square]['piece'] == 'rook':
            for rook in getattr(self, f'{self.current_player}_rook'):
                if rook.position == from_square:
                    return rook.move(self.current_player, game, board[from_square], board[to_square])
        elif board[from_square]['piece'] == 'queen':
            for queen in getattr(self, f'{self.current_player}_queen'):
                if queen.position == from_square:
                    return queen.move(self.current_player, game, board[from_square], board[to_square])
        elif board[from_square]['piece'] == 'king':
            king = getattr(self, f'{self.current_player}_king')
            if king.position == from_square:
                return king.move(self.current_player, game, board[from_square], board[to_square]) 
        return False 
